...
namespace WebBanHangOnline.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
        ...
        public ActionResult Partial_CheckOut(OrderViewModel req)
        {
                var userid = User.Identity.GetUserId();
                var user = db.Users.Find(userid);
                if (user != null)
                {
                    // Gán thông tin người dùng vào OrderViewModel
                    req.CustomerName = string.IsNullOrEmpty(req.CustomerName) ? user.FullName : req.CustomerName;
                    req.Email = string.IsNullOrEmpty(req.Email) ? user.Email : req.Email;
                    req.Phone = string.IsNullOrEmpty(req.Phone) ? user.Phone : req.Phone;
                    req.Address = string.IsNullOrEmpty(req.Address) ? user.Address : req.Address;
            }
        ...
        public ActionResult CheckOut(OrderViewModel req)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    {
                        var sp = db.ProductQuantities.FirstOrDefault(x => x.ProductId == item.ProductId && x.SizeId == item.SizeId && x.ColorId == item.ColorId);
                        var new_qty = sp.QuantityProduct - item.Quantity;
                        sp.QuantityProduct = new_qty;
                        db.SaveChanges();
                    }
                    ...
                    WebBanHangOnline.Common.Common.SendMail("ChangShop", "Đơn hàng #" + order.Code, contentCustomer.ToString(), req.Email);
                    ...
                    WebBanHangOnline.Common.Common.SendMail("ChangShop", "Đơn hàng mới #" + order.Code, contentAdmin.ToString(), ConfigurationManager.AppSettings["EmailAdmin"]);
                    cart.ClearCart();
        ...
        public ActionResult Update(int id, int quantity, int sizeId, int colorId)
        {
            ...
            if (cart != null)
            {
                if(CheckQuantity(id, quantity, sizeId, colorId))
                {
        ...
        [NonAction]
        public bool CheckQuantity(int productId, int quantity, int sizeId, int colorId)
        {
            var isProduct = db.ProductQuantities.FirstOrDefault(x => x.ProductId == productId && x.SizeId == sizeId && x.ColorId == colorId);
            if (isProduct.QuantityProduct <= quantity)
            {
                return false;
            }
            else
        ...
        public ActionResult CancelOrder(int id)
        {
                ...
                {
                order.Status = 4; // Xóa đơn hàng từ cơ sở dữ liệu
                db.SaveChanges();
